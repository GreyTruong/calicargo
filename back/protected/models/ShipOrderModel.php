...
class ShipOrderModel extends CFormModel
{
    ...
    public function update_order($args){
        ...
        $sql = 'update ship_orders set ' . $custom . ' where ship_order_id = :ship_order_id';
        $command = Yii::app()->db->createCommand($sql);
    ...
    public function update_item($args){
        ...
        foreach ($keys as $k)
            $custom .= $k . ' = :' . $k . ', ';
        $custom = substr($custom, 0, strlen($custom) - 2);
        $sql = 'update ship_items set ' . $custom . ' where ship_item_id = :ship_item_id';
        $command = Yii::app()->db->createCommand($sql);
    ...
    public function remove_order($id){
        $item_sql = "DELETE FROM ship_items WHERE ship_order_id = :ship_order_id";
        $command = Yii::app()->db->createCommand($item_sql);
        $command->bindParam(":ship_order_id", $id, PDO::PARAM_INT);
        $command->execute();
        $order_sql = "DELETE FROM ship_orders WHERE ship_order_id = :ship_order_id";
    ...
    public function get_history(){
        $sql = "SELECT * FROM ship_transactions";
        $command = Yii::app()->db->createCommand($sql);
        return $command->queryAll();
    }
    ...
    public function save_transaction($from, $to, $type){
        $lang = "VI";
        $today = time();
        $sql = "INSERT INTO ship_transactions(min_order_id, max_order_id, report_type, report_lang, date_added) ";
    ...
    public function get_all_reported_record(){
        $max = Yii::app()->db->createCommand()->select('max(ship_order_id) as max')->from('ship_orders')->queryScalar();
    ...
    public function is_reported($from, $to){
        ...
        for($i = 0; $i < count($id_arr); $i++)
        {
            $order_id  = $id_arr[$i];
            $sql = "SELECT count(*) as total "; 
            $sql .= "FROM ship_transactions WHERE min_order_id <= :order_id AND max_order_id >= :order_id";
            $command = Yii::app()->db->createCommand($sql);
