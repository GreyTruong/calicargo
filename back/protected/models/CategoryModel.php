...
class CategoryModel extends CFormModel {
    ...
    public function add($title, $slug,$type,$img,$thumbnail,$description,$disabled) {
        ...
        $sql = "INSERT INTO categories(title,slug,type,date_added,img,thumbnail,description,disabled) VALUES(:title,:slug,:type,:date_added,:img,:thumbnail,:description,:disabled)";
        $command = Yii::app()->db->createCommand($sql);
    ...
    private function check_exist_slug($slug) {
        $sql = 'SELECT count(slug) as count FROM categories WHERE slug REGEXP "^' . $slug . '(-[[:digit:]]+)?$"';
        $command = Yii::app()->db->createCommand($sql);
