...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
    ...
    protected function makeThumb($file, $overwrite=true) {
        $this->makeThumbSwap($file, $overwrite);
        $gd = new gd($file);
    ...
    protected function makeThumbSwap($file, $overwrite=true) {
        $gd = new gd($file);
        ...
        $thumb = substr($file, strlen($this->config['uploadDir']));
        $thumb = $this->config['uploadDir'] . "/" . $this->config['thumbsDir'] . "/".$this->config['swap_thumb_dir']."/" . $thumb;
        $thumb = path::normalize($thumb);
        $thumbDir = dirname($thumb);
        if (!is_dir($thumbDir) && !@mkdir($thumbDir, $this->config['dirPerms'], true))
            return false;
        ...
        // Images with smaller resolutions than thumbnails
        if (($gd->get_width() <= $this->config['swap_thumb_width']) &&
            ($gd->get_height() <= $this->config['swap_thumb_height'])
        ) {
//            $browsable = array(IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG);
//            // Drop only browsable types
//            if (in_array($gd->type, $browsable))
//                return true;
             return $gd->imagejpeg($thumb, $this->config['swap_thumb_quality']);

        // Resize image
        } elseif (!$gd->resize_fit($this->config['swap_thumb_width'], $this->config['swap_thumb_height']))
            return false;
