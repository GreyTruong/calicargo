...
class CategoryModel extends CFormModel {
    ...
    public function add($title, $slug,$type,$img,$thumbnail,$description,$disabled) {
        ...
        $sql = "INSERT INTO bc_categories(title,slug,type,date_added,img,thumbnail,description,disabled) VALUES(:title,:slug,:type,:date_added,:img,:thumbnail,:description,:disabled)";
        $command = Yii::app()->db->createCommand($sql);
    ...
    private function check_exist_slug($slug) {
        $sql = 'SELECT count(slug) as count FROM bc_categories WHERE slug REGEXP "^' . $slug . '(-[[:digit:]]+)?$"';
        $command = Yii::app()->db->createCommand($sql);
    ...
    public function update($args) {
        ...
        $sql = 'update bc_categories set ' . $custom . ' where id = :id';
        $command = Yii::app()->db->createCommand($sql);
