...
class FaqModel extends CFormModel {
    ...
    public function add($category_id,$title, $slug,$description,$disabled) {
        ...
        $sql = "INSERT INTO bc_faqs(category_id,title,slug,date_added,description,disabled) VALUES(:category_id,:title,:slug,:date_added,:description,:disabled)";
        $command = Yii::app()->db->createCommand($sql);
    ...
    private function check_exist_slug($slug) {
        $sql = 'SELECT count(slug) as count FROM bc_faqs WHERE slug REGEXP "^' . $slug . '(-[[:digit:]]+)?$"';
        $command = Yii::app()->db->createCommand($sql);
    ...
    public function update($args) {
        ...
        $sql = 'update bc_faqs set ' . $custom . ' where id = :id';
        $command = Yii::app()->db->createCommand($sql);
